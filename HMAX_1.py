...
"""GET C1_inh maps for multiple samples"""
...
N_SAMPLES = 1000
sample_ids = np.random.choice(N_SAMPLES, N_SAMPLES, replace=False)

C1_big = [] # make a list of tuples (C1maps_inh, digit)
G = get_filters(THETAS)           # get the FOUR spatial filters
for sample_id in sample_ids[:2]:
    im = mnist['data'][sample_ids[0]].reshape(28,28)
    ...
    C1_big.append((C1maps_inh, mnist['target'][sample_id]))
